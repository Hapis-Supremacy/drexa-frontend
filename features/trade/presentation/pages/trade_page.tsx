...
} from "@/features/core/presentation/hooks/use_market_stream";
  ...
  return (
    <div
      ...
      onMouseMove={(e) => {
        ...
        const idx = Math.floor((e.clientX - r.left - padL) / step);
    ...
    >
      <svg width={w} height={h} style={{ display: "block" }}>
          ...
          return (
            <g key={i}>
              ...
              <text
                ...
                y={yy + 3.5}
                fill="var(--text-4)"
                style={{ font: "500 10.5px var(--mono)" }}
              >
                {fNum(t, t < 10 ? 3 : 0)}
              </text>
            </g>
        ...
        <text
          ...
          textAnchor="middle"
          fill="#fff"
          style={{ font: "600 10.5px var(--mono)" }}
        >
          {fNum(last, last < 10 ? 3 : 0)}
        </text>
        {data.map((d, i) => {
          const up = d.c >= d.o;
...
function OrderBook({ price, ob }: { price: number; ob?: OrderBookData }) {
  ...
  const maxTot = Math.max(
    1,
    ...[...rows.asks, ...rows.bids].map((r) => r.total),
  );
  const Row = ({
    r,
    side,
  }: {
    r: { p: number; amt: number; total: number };
    side: "ask" | "bid";
  }) => (
    <div
      style={{
    ...
    >
      ...
      <span
        style={{
          position: "relative",
          color: side === "ask" ? "var(--down)" : "var(--up)",
        }}
      >
        {fNum(r.p, r.p < 10 ? 4 : 2)}
      </span>
      <span style={{ position: "relative", color: "var(--text-2)" }}>
        {fNum(r.amt, 4)}
      </span>
    </div>
  );
    ...
    >
      ...
      >
        ...
        <span
          title={
            rows.live
              ? "Live depth from matching engine"
              : "No resting orders — simulated depth"
          }
          style={{
            width: 7,
            height: 7,
            borderRadius: "50%",
            background: rows.live ? "var(--up)" : "var(--text-4)",
            boxShadow: rows.live ? "0 0 0 3px var(--up-soft)" : "none",
          }}
        />
      </div>
      <div
      ...
      >
        ...
        >
          {fNum(price, price < 10 ? 4 : 2)}
        </span>
        <Icon name="arrowUp" size={14} color="var(--up)" stroke={2.4} />
        <span style={{ font: "500 12px var(--font)", color: "var(--text-3)" }}>
          ≈ {fUSD(price, price < 10 ? 4 : 2)}
        </span>
      </div>
      <div>
        {rows.bids.map((r, i) => (
    ...
    <div>
      ...
      <div
        style={{
          ...
          height: 46,
          padding: "0 14px",
          background: readOnly ? "var(--inset)" : "var(--surface)",
          border: "1px solid var(--border)",
          borderRadius: "var(--r-sm)",
        }}
      >
        <input
          ...
          style={{
            ...
            border: "none",
            outline: "none",
            color: readOnly ? "var(--text-2)" : "var(--text-hi)",
            font: "600 15px var(--mono)",
            width: "100%",
          }}
        />
        <span style={{ font: "600 13px var(--font)", color: "var(--text-3)" }}>
          {suffix}
        </span>
      </div>
    </div>
...
function OrderTicket({ coin }: { coin: Coin }) {
  ...
  const [stop, setStop] = useState(
    (coin.price * 0.98).toFixed(coin.price < 10 ? 4 : 2),
  );
  const [limit, setLimit] = useState(
    (coin.price * 1.02).toFixed(coin.price < 10 ? 4 : 2),
  );
  ...
  const hasValidPrice =
    isMarket || (Number.isFinite(priceValue) && priceValue > 0);
    ...
    >
        ...
        {ORDER_TYPES.map((t) => (
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              padding: "0 0 12px",
              font: `${type === t ? 600 : 500} 13px var(--font)`,
              color: type === t ? "var(--text-hi)" : "var(--text-3)",
              borderBottom:
                type === t ? "2px solid var(--blue)" : "2px solid transparent",
              marginBottom: -1,
            }}
          >
            {t}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {type === "Market" ? (
          ...
          type !== "OCO" && (
            <TicketField
              label={type === "Stop-Limit" ? "Limit price" : "Price"}
              value={price}
              onChange={setPrice}
              suffix="USDC"
            />
          ...
          {[25, 50, 75, 100].map((p) => (
            <button
              ...
              style={{
                ...
                border:
                  "1px solid " + (pct === p ? "var(--blue)" : "var(--border)"),
                background: pct === p ? "var(--blue-soft)" : "transparent",
          ...
          {(
            [
              [
                ...
                isBuy
                  ? fUSD(balQuote) + " USDC"
                  : fNum(balBase, 4) + " " + coin.sym,
              ],
              ["Est. fee (0.10%)", fUSD(fee)],
              [
                isBuy ? "You receive" : "You get",
                isBuy ? fNum(amt, 6) + " " + coin.sym : fUSD(total - fee),
              ],
            ] as [string, string][]
          ).map(([k, v]) => (
            <div
            ...
            >
              ...
              <span
                style={{ color: "var(--text-2)", fontFamily: "var(--mono)" }}
              >
                {v}
              </span>
            </div>
        ...
        {placeOrderMutation.isSuccess && (
          ...
          >
            Order placed
            {placeOrderMutation.data?.Status
              ? ` · ${placeOrderMutation.data.Status}`
              : ""}
          </div>
        )}
        ...
        {tabs.map(([id, label]) => (
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              padding: "16px 0",
              font: `${tab === id ? 600 : 500} 13.5px var(--font)`,
              color: tab === id ? "var(--text-hi)" : "var(--text-3)",
              borderBottom:
                tab === id ? "2px solid var(--blue)" : "2px solid transparent",
              marginBottom: -1,
            }}
          >
            {label}
            {id === "open" ? ` (${OPEN_ORDERS.length})` : ""}
          </button>
        ))}
      </div>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <>
            ...
            <tbody>
              {OPEN_ORDERS.map((o, i) => (
                ...
                >
                  <td
                    style={{ ...td, color: "var(--text-hi)", fontWeight: 600 }}
                  >
                    {o.pair}
                  </td>
                  <td style={td}>
                  ...
                  <td style={{ ...tdM, textAlign: "right" }}>
                    {fNum(o.price, o.price < 10 ? 4 : 2)}
                  </td>
                  <td style={{ ...tdM, textAlign: "right" }}>{o.amt}</td>
                  <td style={{ ...tdM, textAlign: "right" }}>{o.filled}</td>
                  <td
                    style={{
                      ...tdM,
                      textAlign: "right",
                  ...
                  <td style={{ ...td, textAlign: "right" }}>
                    <span
                      style={{
                        ...
                        color:
                          o.status === "Filled" ? "var(--up)" : "var(--text-3)",
                      }}
                    >
                      {o.status}
                    </span>
        ...
        {tab === "trades" && (
          <>
            <thead>
              <tr>
                {["Pair", "Side", "Price", "Amount", "Fee", "Time"].map(
                  (h, i) => (
                    <th
                      key={i}
                      style={{
            ...
            <tbody>
              {TRADE_HISTORY.map((o, i) => (
                ...
                >
                  ...
                  <td style={{ ...tdM, textAlign: "right" }}>{o.amt}</td>
                  <td style={{ ...tdM, textAlign: "right" }}>{fUSD(o.fee)}</td>
                  <td
                    style={{
                      ...tdM,
                      textAlign: "right",
...
export function TradePage({ sym: symProp }: { sym?: string }) {
  ...
  const { candles: data, loading: chartLoading } = useBinanceKlines(
    sym,
    tf,
    TF_LIMIT[tf] ?? 100,
  );
  ...
  const stats: [string, string][] = [
    ["24h High", fNum(hi, coin.price < 10 ? 4 : 2)],
    ["24h Low", fNum(lo, coin.price < 10 ? 4 : 2)],
    ["24h Volume", fCompact(coin.vol)],
  ];
  return (
    <AppShell>
          ...
          <div style={{ position: "relative" }}>
            ...
            >
              ...
              <Icon
                ...
                size={18}
                color="var(--text-3)"
                style={{ transform: pairOpen ? "rotate(180deg)" : "none" }}
              />
            </button>
            {pairOpen && (
              <div
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span
                style={{
                  ...
                  color: coin.ch >= 0 ? "var(--up)" : "var(--down)",
                  fontVariantNumeric: "tabular-nums",
                }}
              >
                {fUSD(coin.price, coin.price < 10 ? 4 : 2)}
              </span>
              <span
                ...
                style={{
                  ...
                  height: 7,
                  borderRadius: "50%",
                  background: isConnected ? "var(--up)" : "var(--text-4)",
                  boxShadow: isConnected ? "0 0 0 3px var(--up-soft)" : "none",
                }}
              />
            </div>
            <div style={{ marginTop: 2 }}>
              <Delta v={coin.ch} size={13} icon />
            </div>
          </div>
                ...
                {TFS.map((t) => (
                  <button
                    ...
                    style={{
                      ...
                      border: "none",
                      cursor: "pointer",
                      background: tf === t ? "var(--card-2)" : "transparent",
                      color: tf === t ? "var(--text-hi)" : "var(--text-3)",
