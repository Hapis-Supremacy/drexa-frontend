...
function StripeCheckoutForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (confirmError) {
      ...
      try {
        await api.post("/payments/deposit/verify", {
    ...
    >
      <PaymentElement
        onLoadError={(e) => {
          ...
          setError(
            e.error?.message ||
              "Failed to load payment form. Please check your Stripe configuration or try again.",
          );
        }}
      />
      {error && (
      ...
      <button
        ...
        style={{
          ...
          background: loading || !stripe ? "var(--card-2)" : "var(--blue)",
          color: loading || !stripe ? "var(--text-3)" : "#fff",
      ...
      >
        <Icon
          name="lock"
          size={15}
          color={loading || !stripe ? "var(--text-4)" : "#fff"}
        />
      ...
      {!clientSecret ? (
        <>
          <div style={{ marginBottom: 14 }}>
            ...
            >
              <span
                style={{ font: "600 15px var(--mono)", color: "var(--text-3)" }}
              >
                $
              </span>
              <input
                value={buyAmt}
                onChange={(e) =>
                  setBuyAmt(e.target.value.replace(/[^0-9.]/g, ""))
                }
                placeholder="100.00"
                inputMode="decimal"
                style={{
          ...
          >
            ...
            <SummaryRow
              ...
              v={
                buyAmt
                  ? fUSD((parseFloat(buyAmt) || 0) * 0.985) + " USD"
                  : "\u2014"
              }
            />
          </div>
          ...
          <button
            ...
            disabled={!buyAmt || loadingIntent}
            style={{
              width: "100%",
              height: 48,
              borderRadius: "var(--r-md)",
              border: "none",
              cursor: !buyAmt || loadingIntent ? "default" : "pointer",
              background:
                !buyAmt || loadingIntent ? "var(--card-2)" : "var(--blue)",
              color: !buyAmt || loadingIntent ? "var(--text-3)" : "#fff",
              font: "700 14.5px var(--font)",
            }}
          >
...
function WithdrawModalContent({ rows, closeModal }: any) {
  ...
  const usdRow = rows.find((r: any) => r.sym === "USD" || r.sym === "USDC") || {
    qty: 0,
  };
  ...
  if (success) {
    return (
      <div style={{ textAlign: "center", padding: "20px 0" }}>
        ...
        >
          Your funds have been instantly sent to your PayPal account.
        </p>
        <button
          onClick={closeModal}
          style={{
    ...
    <>
      <div style={{ marginBottom: 14 }}>
        ...
        >
          ...
          />
          <button
            onClick={() => setPayoutAmt(maxUsd.toString())}
            style={{
      ...
      >
        ...
        <SummaryRow
          k="You receive"
          v={payoutAmt ? `$${fNum(parseFloat(payoutAmt) || 0, 2)}` : "—"}
        />
      </div>
      {error && (
        <div
      ...
      <button
        ...
        disabled={!payoutAmt || !paypalEmail || loading}
        style={{
          width: "100%",
          height: 48,
          borderRadius: "var(--r-md)",
          border: "none",
          cursor: !payoutAmt || !paypalEmail || loading ? "default" : "pointer",
          background:
            !payoutAmt || !paypalEmail || loading
              ? "var(--card-2)"
              : "var(--blue)",
          color:
            !payoutAmt || !paypalEmail || loading ? "var(--text-3)" : "#fff",
          font: "700 14.5px var(--font)",
        }}
      >
  ...
  if (success) {
    return (
      <div style={{ textAlign: "center", padding: "20px 0" }}>
        ...
        >
          Funds have been sent to {toUserId}.
        </p>
        <button
          onClick={closeModal}
          style={{
    ...
    <>
      ...
      <div style={{ marginBottom: 2 }}>
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div
              style={{ font: "600 13px var(--font)", color: "var(--text-hi)" }}
            >
              My Drexa Wallet
            </div>
          </div>
          ...
          >
            {fNum(coinRow.qty, coinRow.qty < 1 ? 4 : 2)} {asset}
          </span>
        </div>
      </div>
      <div
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div
          style={{
            ...
            border:
              "1px solid " +
              (toUserId.length > 10 ? "var(--blue)" : "var(--border)"),
            borderRadius: "var(--r-sm)",
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        >
          <input
            value={txAmt}
            onChange={(e) => setTxAmt(e.target.value.replace(/[^0-9.]/g, ""))}
            placeholder="0.00"
            inputMode="decimal"
            style={{
              flex: 1,
          ...
          />
          <button
            onClick={() => setTxAmt(fNum(coinRow.qty, coinRow.qty < 1 ? 6 : 4))}
            style={{
              font: "600 12px var(--font)",
              color: "var(--blue-hover)",
              background: "none",
          ...
          <span
            style={{ font: "600 13px var(--font)", color: "var(--text-3)" }}
          >
            {asset}
          </span>
        </div>
      ...
      <button
        ...
        disabled={!toUserId || !txAmt || loading}
        style={{
          width: "100%",
          height: 48,
          borderRadius: "var(--r-md)",
          border: "none",
          cursor: !toUserId || !txAmt || loading ? "default" : "pointer",
          background:
            !toUserId || !txAmt || loading ? "var(--card-2)" : "var(--blue)",
          color: !toUserId || !txAmt || loading ? "var(--text-3)" : "#fff",
          font: "700 14.5px var(--font)",
          transition: "background .15s, color .15s",
        }}
      >
      ...
      >
        ...
        >
          {icon && (
            ...
            >
              <Icon name={icon} size={19} color="var(--blue-hover)" />
            </span>
          )}
          <div
            style={{
...
function AssetSelect({
  ...
  setNet: Dispatch<SetStateAction<number>>;
}) {
  return (
...
export function WalletPage() {
  ...
  const rows = useMemo(() => {
    ...
    DEFAULT_ASSETS.forEach((sym) => {
      ...
      baseMap.set(sym, {
        ...
        inOrders: 0,
        available: 0,
        name: c ? c.name : sym === "USDT" ? "Tether" : sym,
      });
    });
  ...
  const allTxns = useMemo(() => {
    return apiTxns.map((t) => ({
      ...
      amt:
        t.amount /
        (t.currency.toUpperCase() === "BTC"
          ? 100_000_000
          : t.currency.toUpperCase() === "ETH"
      ...
      status:
        t.status === "completed"
          ...
          : t.status === "failed"
            ...
            : "Pending",
      net: "Drexa",
      time: new Date(t.createdAt).toLocaleString(undefined, {
        month: "short",
        day: "numeric",
        hour: "2-digit",
        minute: "2-digit",
  ...
  const openModal = (type: string, sym?: string) => {
    setModal(type);
    if (sym) {
      setAsset(sym);
      setNet(0);
  ...
  useEffect(() => {
    ...
    if (clientSecret && paymentIntentId && redirectStatus === "succeeded") {
      ...
      api
        .post("/payments/deposit/verify", { provider_ref: paymentIntentId })
        .then(() => {
          refresh();
        })
  ...
  return (
    <AppShell>
      <Container max={1200} style={{ padding: "36px 32px 64px" }}>
        ...
        >
          Manage your balances, deposits, withdrawals, and transfers.
        </p>
            ...
            {(
              ...
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  font: "600 14px var(--font)",
                  border: id === "deposit" ? "none" : "1px solid var(--border)",
                  background:
                    id === "deposit" ? "var(--blue)" : "var(--surface)",
                  color: id === "deposit" ? "#fff" : "var(--text)",
                }}
              >
                <Icon
                  name={ic}
                  size={17}
                  color={id === "deposit" ? "#fff" : "var(--text-2)"}
                />
                {label}
              </button>
            ))}
        ...
        >
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                {["Asset", "Total", "Available", "Value", ""].map((h, i) => (
                  <th
                    key={i}
                    style={{
            ...
            <tbody>
              {rows.map((r) => (
                ...
                >
                  ...
                  <td style={tdWm}>
                    {fNum(r.available / r.price, r.qty < 1 ? 5 : 2)}
                  </td>
                  <td
                    style={{
                      ...tdWm,
                  ...
                  <td style={{ textAlign: "right", padding: "16px 24px" }}>
                    <div style={{ display: "inline-flex", gap: 6 }}>
                      <button
                        onClick={() => openModal("deposit", r.sym)}
                        className="wal-act"
                      >
                        Deposit
                      </button>
                      <button
                        onClick={() => openModal("withdraw", r.sym)}
                        className="wal-act"
                      >
                        Withdraw
                      </button>
                      <button
                        onClick={() => openModal("transfer", r.sym)}
                        className="wal-act"
                      >
                        Transfer
                      </button>
                  ...
                  >
                    <td style={{ padding: "14px 24px" }}>
                      ...
                      >
                        ...
                        >
                          <Icon
                            name={
                              isDep ? "deposit" : isWd ? "withdraw" : "transfer"
                            }
                            size={15}
                            color={col}
                          />
                    ...
                    <td
                      style={{
                        ...
                        color: isDep ? "var(--up)" : "var(--text-hi)",
                        fontVariantNumeric: "tabular-nums",
                      }}
                    >
                      {isDep ? "+" : isWd ? "−" : ""}
                      {fNum(t.amt, t.amt < 1 ? 5 : 2)}
                    </td>
                    <td
                      style={{
                        padding: "14px 24px",
